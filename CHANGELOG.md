...
- **The registry catches up with CCP**: 401 type ids to 539. IV-Grade for all fifteen classic ores, the Exordium 0-Grade tier, four X-Grade families, Prismaticite, nine gas colours plus 27 compressed gas types and Fullerite-C32, base and compressed Rakovene and Bezdnacine, and 63 batch-compressed variants recognised but deliberately unpriced. Comments updated for CCP's rename of every ore variant to the numeric scheme.
...
- New **Price Provider Trouble**: says so once when price refreshes stop getting through and once when they work again. Per-webhook opt-in, off by default.
...
- Split pricing reads the prices Janice actually returns and falls back to whichever side of the market has orders, so moon ore that only trades one way is no longer valued at half of nothing.
...
- The tax pipeline check gains **Step 4b: Payment Reconciliation**, checking that each invoice settled since the cutover matches the payments recorded against it.
- **Data Integrity** flags account balances that cannot be right, refunds whose balance row is gone, and ore that was mined but has no price and no reprocessing materials to fall back on.
