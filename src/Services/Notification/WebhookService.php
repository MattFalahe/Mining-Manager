...
class WebhookService
{
    ...
    /**
     * Get the Discord role mention for a notification type.
     * Uses per-type settings first, falls back to webhook-level role.
     *
     * @param string $eventType Event type key (e.g., 'theft_detected', 'moon_arrival')
     * @param WebhookConfiguration $webhook Webhook for fallback
     * @return string|null Discord role mention string or null
     */
    protected function getDiscordRoleMention(string $eventType, WebhookConfiguration $webhook): ?string
    {
        // Map webhook event types to settings type keys
        $settingsKey = match ($eventType) {
            'moon_arrival' => 'moon_ready',
            default => $eventType, // Most event types match directly
        };
    ...
    protected function sendToDiscord(WebhookConfiguration $webhook, TheftIncident $incident, string $eventType, array $additionalData): array
    {
        ...
        // Add role mention from per-type settings or webhook fallback
    ...
    public function testWebhook(WebhookConfiguration $webhook): array
    {
        ...
        // Use webhook's corporation_id so the test matches this webhook's scope
    ...
    protected function buildMoonDiscordEmbed(string $eventType, array $data): array
    {
        ...
        if ($eventType === 'jackpot_detected') {
            ...
            if (isset($data['auto_fracture_time'])) {
                $embed['fields'][] = [
                    'name' => '⏰ Est. Auto Fracture',
                    'value' => $data['auto_fracture_time'],
                    'inline' => true,
                ];
            }
            ...
            if (isset($data['extraction_url'])) {
                $embed['fields'][] = [
                    ...
                    'value' => '[View Extraction](' . $data['extraction_url'] . ')',
                    'inline' => false,
                ];
            }
    ...
    protected function buildTaxDiscordEmbed(string $eventType, array $data): array
    {
        ...
        $showAmount = $data['show_amount'] ?? true;
        $baseUrl = rtrim(config('app.url', ''), '/');
        $myTaxesUrl = $data['my_taxes_url'] ?? $baseUrl . '/mining-manager/tax/my-taxes';
        $helpUrl = $data['help_url'] ?? $baseUrl . '/mining-manager/help#how-to-pay';

        // Period (for announcements and generated summaries)
        ...
        // Amount (conditional on show_amount setting)
        ...
        $embed['fields'][] = [
            'name' => '❓ How to Pay',
            'value' => '[Payment Guide](' . $helpUrl . ')',
            'inline' => true,
        ];

        // Description
        $embed['description'] = match ($eventType) {
            'tax_announcement' => "Mining tax invoices have been generated for this period.\nCheck **My Taxes** to view your invoice and payment instructions.",
    ...
    protected function sendTaxToSlack(WebhookConfiguration $webhook, string $eventType, array $data): array
    {
        ...
        $blocks[] = [
            'type' => 'section',
            'fields' => [
                ['type' => 'mrkdwn', 'text' => "*My Taxes:*\n<{$myTaxesUrl}|View My Taxes>"],
                ['type' => 'mrkdwn', 'text' => "*How to Pay:*\n<{$helpUrl}|Payment Guide>"],
            ],
        ];
    ...
    {
        if (!$this->isEventTypeEnabled('report_generated')) {
            return ['sent' => 0, 'failed' => 0, 'message' => 'Event type globally disabled'];
        }
    ...
    protected function sendReportToDiscord(WebhookConfiguration $webhook, MiningReport $report, array $reportData): array
    {
        ...
        $roleMention = $this->getDiscordRoleMention('report_generated', $webhook);
        if ($roleMention) {
            $payload['content'] = $roleMention;
        }
    ...
    protected function getColorForEventType(string $eventType): int
    {
        return match($eventType) {
            ...
            'tax_announcement' => 0x3498DB,      // Blue
    ...
    protected function getTitleForEventType(string $eventType): string
    {
        return match($eventType) {
            ...
            'tax_announcement' => '📢 New Mining Invoices',
    ...
    /**
     * Check if a notification event type is globally enabled.
     * Reads from notification settings. Unknown types default to enabled (backward compatible).
     */
    private function isEventTypeEnabled(string $eventType): bool
    {
        try {
            ...
            $enabledTypes = $settings->getSetting('notifications.enabled_types', []);

            // If no types are configured, all are enabled (backward compatible)
            if (empty($enabledTypes)) {
                return true;
            }

            return (bool) ($enabledTypes[$eventType] ?? true);
        } catch (\Exception $e) {
