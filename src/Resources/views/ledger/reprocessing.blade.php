    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="card card-dark">
                ...
                @if(isset($cacheInfo))
                <div class="alert alert-{{ $cacheInfo['is_stale'] ? 'warning' : 'info' }} py-2 mb-3 mx-3 mt-3" style="font-size: 0.85em;">
                    <i class="fas fa-{{ $cacheInfo['is_stale'] ? 'exclamation-triangle' : 'database' }}"></i>
                    <strong>Price Cache:</strong>
                    @if($cacheInfo['oldest_age_minutes'] !== null)
                        Prices last updated {{ $cacheInfo['oldest_age_minutes'] }} minutes ago
                        @if($cacheInfo['is_stale'])
                            <span class="text-warning">(stale — run <code>mining-manager:cache-prices</code> to refresh)</span>
                        @endif
                    @else
                        <span class="text-danger">No cached prices available</span>
                    @endif
                </div>
                @endif
                <div class="card-body">
...
$(document).ready(function() {
    ...
    function renderResults(data) {
        ...
        // Show price cache info
        if (data.summary.price_cache_age_minutes !== null && data.summary.price_cache_age_minutes !== undefined) {
            var ageText = data.summary.price_cache_age_minutes + ' minutes ago';
            var staleClass = data.summary.price_cache_stale ? 'text-warning' : 'text-info';
            var staleIcon = data.summary.price_cache_stale ? 'exclamation-triangle' : 'clock';
            $('#priceAgeInfo').html(
                '<i class="fas fa-' + staleIcon + ' ' + staleClass + '"></i> ' +
                '<small class="' + staleClass + '">Prices cached ' + ageText +
                (data.summary.price_cache_stale ? ' (stale)' : '') + '</small>'
            );
        }
    }
