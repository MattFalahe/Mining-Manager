<form method="POST" action="{{ route('mining-manager.settings.update-notifications') }}">
    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- METENOX CARGO FULL THRESHOLD                                    --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="card bg-dark mb-3">
        <div class="card-header" style="background: linear-gradient(135deg, #2d3748 0%, #4a5568 100%);">
            <h5 class="card-title mb-0">
                <i class="fas fa-box-open text-warning"></i>
                Metenox Cargo Bay Full Threshold
            </h5>
        </div>
        <div class="card-body">
            <small class="form-text text-muted mb-3 d-block">
                Fill % at which the <code>metenox_cargo_full</code> notification fires when a Metenox
                Moon Drill's <code>MoonMaterialBay</code> crosses going up. Default <strong>85%</strong> &mdash;
                at typical Metenox production (~1,500-2,000 m³/hour) the 500,000 m³ bay caps in 10-14
                days, so 85% gives operators about 2 days of lead time before drilling stops. Lower
                = more aggressive warning (longer lead time); higher = tighter (less spam, less
                warning). Clamped to <code>50-99</code>.
            </small>
            <div class="form-group row align-items-center mb-0">
                <label for="metenox_cargo_full_threshold_pct" class="col-md-4 col-form-label">
                    Fill threshold for the alert
                </label>
                <div class="col-md-3">
                    <div class="input-group">
                        <input type="number"
                               class="form-control"
                               id="metenox_cargo_full_threshold_pct"
                               name="metenox_cargo_full_threshold_pct"
                               min="50" max="99" step="1"
                               value="{{ old('metenox_cargo_full_threshold_pct', $notificationSettings['metenox_cargo_full_threshold_pct'] ?? 85) }}">
                        <div class="input-group-append">
                            <span class="input-group-text">% full</span>
                        </div>
                    </div>
                    @error('metenox_cargo_full_threshold_pct')
                ...
                <div class="col-md-5">
                    <small class="text-muted">
                        <i class="fas fa-info-circle"></i>
                        Each Metenox crossing the threshold fires <strong>once</strong> per fill
                        cycle; the latch resets when cargo is pulled (fill drops below threshold).
                    </small>
                </div>
            </div>
        </div>
